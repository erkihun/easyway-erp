...
@section('content')
<x-ui.page-header :title="__('users.edit')" :subtitle="__('users.edit_subtitle', ['name' => $user->name])" icon="heroicon-o-pencil-square">
    <x-slot:actions>
        <x-ui.page-actions>
...
<form method="POST" action="{{ route('admin.users.update',$user) }}">
    ...
    <x-ui.card class="mb-1">
        ...
        <div class="row">
            <x-ui.input name="name" :label="__('common.name')" :value="$user->name" required />
            <x-ui.input name="email" :label="__('common.email')" type="email" :value="$user->email" required />
        </div>
    </x-ui.card>
    ...
    <x-ui.card class="mb-1">
        ...
        <div class="row">
            <x-ui.input name="password" :label="__('users.password_new')" type="password" :help="__('users.password_keep_help')" />
            <x-ui.input name="password_confirmation" :label="__('users.password_confirm_new')" type="password" />
        </div>
    </x-ui.card>
    ...
    @else
        <x-ui.card>
            <x-slot:header><h3 class="table-shell-title">{{ __('users.roles') }}</h3></x-slot:header>
            <p class="muted">{{ __('users.role_modify_forbidden') }}</p>
        </x-ui.card>
    @endcanany
