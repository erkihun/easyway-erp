...
@section('title', __('invoice.create_invoice'))
@section('page-title', __('invoice.create_invoice'))
@section('page-subtitle', __('invoice.subtitle'))
@section('content')
<x-ui.page-header :title="__('invoice.create_invoice')" :subtitle="__('invoice.create_subtitle')" icon="heroicon-o-plus-circle" />

<form method="POST" action="{{ route('admin.invoices.store') }}"
      x-data="invoiceForm({
        salesOrders: @js($salesOrdersPayload),
        selectedSalesOrderId: @js((string) old('sales_order_id', '')),
        initialLines: [],
      })">
    @csrf
        ...
        <div class="space-y-4">
                    ...
                    <div class="row">
                        <x-ui.select name="sales_order_id" :label="__('invoice.sales_order')" x-model="selectedSalesOrderId" x-on:change="applySalesOrder()">
                            ...
                            @foreach($salesOrders as $s)
                                <option value="{{ $s->id }}">{{ $s->order_number }}{{ $s->customer ? ' - '.$s->customer->name : '' }}</option>
                            @endforeach
                        </x-ui.select>
                        <x-ui.input type="date" name="invoice_date" :label="__('invoice.invoice_date')" :value="old('invoice_date', now()->toDateString())" required />
                        <x-ui.input type="date" name="due_date" :label="__('invoice.due_date')" :value="old('due_date', now()->addDays(15)->toDateString())" />
                        ...
                        <x-ui.select name="status" :label="__('common.status')">
                            <option value="draft">{{ __('invoice.draft') }}</option>
                            <option value="issued">{{ __('invoice.sent') }}</option>
                        </x-ui.select>
                    </div>
                    <x-ui.textarea name="notes" :label="__('invoice.customer_notes')" rows="2" />
                </div>
            </div>
            ...
            <div class="panel">
                <div class="panel-body">
                    ...
                    <p class="muted mt-1">{{ __('invoice.line_items_help') }}</p>
                </div>
            </div>
        </div>
    ...
    <div class="form-actions-sticky mt-1">
        ...
        <x-ui.button type="submit" icon="heroicon-o-check">{{ __('invoice.create_invoice') }}</x-ui.button>
    </div>
</form>
@endsection
